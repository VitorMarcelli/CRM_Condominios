...
import { ArrowLeft, Save, X, Siren, Loader2, Workflow, Hash, ShieldCheck } from 'lucide-react';
    ...
    <div className="space-y-8 pb-12 w-full max-w-[800px] mx-auto font-sans">
      <div className="flex items-center gap-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-white">Editar Regra</h1>
            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mt-1">Atualize as configurações da regra <span className="font-bold">{name}</span>.</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#151515] rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 dark:border-white/5 overflow-hidden p-6 md:p-10">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="col-span-1 md:col-span-2 pt-4 border-t border-slate-100 dark:border-white/5">
              <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-black/20 rounded-xl border border-slate-200 dark:border-white/10 w-fit">
                <input 
                  type="checkbox" 
                  id="isActive" 
                  checked={isActive} 
                  onChange={e => setIsActive(e.target.checked)} 
                  className="h-5 w-5 rounded border-slate-300 dark:border-slate-700 text-red-600 focus:ring-red-500 bg-white dark:bg-black" 
                />
